...
from benji.k8s_operator.crd.version import LABEL_INSTANCE
...
from benji.k8s_operator.constants import LABEL_PARENT_KIND, SCHED_VERSION_RECONCILIATION_JOB, \
    SCHED_CLEANUP_JOB, API_VERSION, API_GROUP
...
def reconciliate_versions_job(*, logger):
    ...
    for version_resource in BenjiVersion(kubernetes_client).objects().filter(selector=f'{LABEL_INSTANCE}={benji_instance}'):
